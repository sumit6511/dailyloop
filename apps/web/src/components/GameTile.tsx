...
export type GameTileState = "default" | "selected" | "correct" | "present" | "absent" | "given" | "disabled";
...
export function GameTile({ state = "default", size = "md", className = "", disabled, ...props }: GameTileProps) {
  return (
    <button
      ...
      className={`flex items-center justify-center rounded-xl border-2 font-bold uppercase transition-colors disabled:cursor-not-allowed ${STATE_CLASSES[state]} ${SIZE_CLASSES[size]} ${className}`}
      {...props}
    />
  );
}
